...
#### 非对称算法
好吧，这就是现代密码学，还是相距千里的间谍A和间谍B进行交流：间谍A有两个密码，分别为P和C，密码P预先给了间谍B（怎么给都行，被别人看到也行），间谍B把信息用密码P加密，然后把信息再发给间谍A，然后间谍A用自己手里的密码C把信息解密，打开一看：哦，原来间谍B告诉我是要告诉我这么个破事啊。
**这块小编要说说：**
...
#### 余数的计算
`ps:一般mod运算的目的就是求出最小的`$\gamma$`值，即我们学生时代所学的余数。`（看完后文的等价类就知道为什么要有个最小的了）
