...
```
由于要使用到大量的数据集，所有有两个指向lru的指针。并且通过threads控制挖矿线程数。并在测试模式或fake模式下，简单快速处理，使之快速得到结果。

## 共识算法接口实现
上一篇文章我们得知，共识算法是实现`Engine接口`中等方法。也就是说结构体`Ethash`要实现其中的每一个接口。
...
4. 获取到父块后，进一步对该新块做更细的校验。
    ...
    2. 判断该块是否为叔块（true、false）。
        ...
        2. 该块若不是蜀块，则判断该块时间戳是否超过当前时间（+15秒，可得知`每15秒左右出一个块`），若是则返回异常，否则继续
...
这个方法用于计算下一个块的难度，同时，从上面校验方法我们可知，
可从以下代码看出：
```go
// 具体的实现需要一层层进入
...
func (ethash *Ethash) VerifyHeaders(chain consensus.ChainReader, headers []*types.Header, seals []bool) (chan<- struct{}, <-chan error) {
	...
	errorsOut := make(chan error, len(headers)) //用于再select接收存储到的错误
...
这块我们可以得知以下几个重要信息：
1. 一个块有两个叔块
2. 将该块的前8个块及其叔块信息分别汇总，是为了更好的确保当前块叔块的正确性，防止有重复等问题，确保叔块和正常块不会交叉混淆，这也是避免有人恶意操作吧。
...
### Finalize()
非常重要的一个方法，通俗的讲，就是分配奖励给`区块账户`和`叔块账户`。
