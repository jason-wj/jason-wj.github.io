...
#### server所拥有的方法
server.go文件中，剩下的内容都是server类（或者称为结构体）所对应的方法，都比较重要，我们一个个来介绍
...
##### serveRequest()处理客户端发来的请求
serveRequset()方法用来处理codec中的请求内容，服务器使用codec读取请求，处理请求，再使用它返回请求给客户端。


说`serveRequest()`前需要先说`ServeCodec()`和`ServeSingleRequest()`这两个方法：
* `ServeCodec()`方法会调用`serveRequest()`方法来处理codec传入的请求信息，除非codec请求关闭或者server本身停止，否则该方法不会结束。若结束时候，会调用codec.Close()。
```go
